...
if (spec := util.find_spec('pikepdf')) is not None:
    ...
    class GlyphlessFont(EncodableFont):
        ...
        GLYPHLESS_FONT = (package_files('pageplus.utils.pdf.fonts') / GLYPHLESS_FONT_NAME).read_bytes()
        CHAR_ASPECT = 2
        ...
        def calculate_fontsize(self, text: str, width: float) -> float:
            """Estimate the width of a text string when rendered with the given font."""
            # NFKC: split ligatures, combine diacritics
            return (width*self.CHAR_ASPECT)/len(unicodedata.normalize("NFKC", text))
