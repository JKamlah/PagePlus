"""PP-Layout Extended Table JSON → PAGE XML mapping.

Converts extended PP-Layout JSON (with 'regions' and 'lines') to PAGE XML,
mapping table regions to TableRegion with nested TableCell/TextLine elements.
"""
import json
import logging
...
def pp_layout_extend_table_json_to_page(
    data: Any,
    image_path: Path,
    settings: Optional[dict] = None,
    **kwargs,
) -> str:
    """Convert extended PP-Layout JSON to PAGE XML, retaining TableRegion structures.

    Expected JSON schema same as pp_layout_extend_json_to_page.
    """
    ...
    try:
        ...
        logging.error(f"Error reading image dimensions for PP-Layout Extend Table: {e}")
        return ""

    if not isinstance(data, dict):
        logging.error("PP-Layout Extend Table JSON is not a dictionary.")
        return ""
    ...
    page_xml_lines = page_xml_header(
        creator="PagePlus - PP-Layout-Extend-Table-JSON",
        image_path=image_path,
        img_width=img_width,
        img_height=img_height,
        comment="Generated from PP-Layout-Extend-Table-JSON",
    )
    ...
    for idx, entry in enumerate(regions):
        ...
        if not coords_tuples:
            logging.warning(f"PP-Layout Extend Table region {region_id_val} has no usable coordinates; skipping.")
            continue
        ...
        if region_lines:
            ...
            elif region_tag == "TableRegion":
                cell_xml_id = f"{xml_region_id}_c0"
                page_xml_lines.append(f'            <TableCell id="{cell_xml_id}" row="0" col="0" rowSpan="1" colSpan="1">')
                page_xml_lines.append(f'                <Coords points="{coords_str}"/>')
                for l_idx, line in enumerate(region_lines):
                    line_coords_tuples = _parse_line_coords(line)
                    if not line_coords_tuples:
                        continue
                    line_coords_str = " ".join(f"{x},{y}" for x, y in line_coords_tuples)
                    line_xml_id = f"{cell_xml_id}_l{l_idx + 1}"
                    ys = [pt[1] for pt in line_coords_tuples]
                    xs = [pt[0] for pt in line_coords_tuples]
                    baseline_y = int(max(ys) - (max(ys) - min(ys)) * 0.2)
                    baseline_str = f"{min(xs)},{baseline_y} {max(xs)},{baseline_y}"
    ...
    if ordered_refs_to_use:
        page_xml_lines_ro = [
            "        <ReadingOrder>",
            '            <OrderedGroup id="ro_pp_layout_extend_table" caption="Regions reading order">',
        ]
