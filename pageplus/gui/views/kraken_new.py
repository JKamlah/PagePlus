...
def _show_cli_mode(cli_bridge, selected_files):
    ...
    **Native Kraken CLI Functions**
    ...
    seg_model_name, rec_model_name, model_dir = _show_model_configuration(
        cli_bridge, processing_mode, key_prefix="cli"
    )

    # Performance Options
    device, text_direction, jobs, template, threads = _show_performance_options(key_prefix="cli", mode=processing_mode)
    ...
    if st.button(button_text[processing_mode], type="primary", key="cli_process_btn"):
        ...
        _run_cli_processing(
            ...
            seg_model_name, rec_model_name, model_dir, device, text_direction, text_direction, jobs, template, threads
        )
...
def _show_pageplus_mode(cli_bridge, selected_files):
    ...
    **PagePlus Advanced Functions**
    ...
    # Performance options for PagePlus mode
    ...
    jobs = st.number_input(
        ...
        max_value=16,
        value=1,
        help="Number of parallel jobs for OCR processing.",
        key="pageplus_jobs"
    )
...
def _show_model_configuration(cli_bridge, processing_mode, key_prefix=""):
    ...
    if model_dir_input:
        ...
        if model_dir.exists():
            available_models = cli_bridge.get_available_models(model_dir)
    ...
    return seg_model_name, rec_model_name, model_dir
...
def _show_performance_options(key_prefix="", mode=None):
    ...
    with col2:
        # Different text-direction options based on mode
        if mode == "Recognition Only (requires XML)":
            # OCR text-direction options
            text_direction_options = ["horizontal-tb", "vertical-lr", "vertical-rl"]
            default_direction = "horizontal-tb"
            help_text = "Text direction for OCR output"
        else:
            # Segment text-direction options (also used for combined mode)
            text_direction_options = ["horizontal-lr", "horizontal-rl", "vertical-lr", "vertical-rl"]
            default_direction = "horizontal-lr"
            help_text = "Text direction for segmentation"

        text_direction = st.selectbox(
            "Text Direction",
            options=text_direction_options,
            index=text_direction_options.index(default_direction),
            help=help_text,
            key=f"{key_prefix}_text_direction"
        )
    ...
    return device_option, text_direction, jobs, template, threads
