...
# Assuming these are your custom classes based on the import paths you
# provided.
from pageplus.models.basic_elements import Region
from pageplus.models.text_elements import Textline, TextRegion
...
class TableCell(TextRegion):
    ...
    """
    parent: Optional[Region] = field(
        default=None)  # Modified to directly use None as default
...
class TableRegion(Region):
    ...
    """
    parent: Optional[None] = field(
        default=None)  # Modified to directly use None as default
    tablecells: List[TableCell] = field(
        default_factory=list)  # List of TableCell instances
    textlines: List[Textline] = field(
        default_factory=list)  # List of Textline instances

    def __post_init__(self):
        """
        Initializes the TableRegion by extracting TableCell elements and their text lines.
        """
        # Assuming `self.xml_element` and `self.ns` are defined in the `Region`
        # base class
        ...
        for tc in self.tablecells:
            ...
            self.textlines.extend(tc.textlines)
